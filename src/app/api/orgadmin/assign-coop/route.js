...
export async function POST(request) {
  ...
  try {
    ...
    const allowedAssignmentRoles = new Set(["auditer", "aud_E", "aud_T"]);
    ...
    // Check existing auditor
    const existingAuditorResponse = await databases.listDocuments(
      DATABASE_ID,
      COLLECTION_ID_TEAM_X_COOP,
      [Query.equal("coopId", coopId), Query.equal("role", "auditer")],
    );

    const hasExistingAuditor = existingAuditorResponse.documents.length > 0;

    const incomingAuditors = members.filter((m) => m.role === "auditer");

    if (incomingAuditors.length > 1) {
      return NextResponse.json(
        {
          success: false,
          error: "Only one auditor can be assigned",
        },
        { status: 400 },
      );
    }

    if (hasExistingAuditor && incomingAuditors.length > 0) {
      return NextResponse.json(
        {
          success: false,
          error: "There is already an assigned auditor for this coop",
        },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      console.error("Failed to write assignment audit log:", logErr);
    }
...
export async function DELETE(request) {
  ...
  try {
    ...
    // If the removed member was an auditor, also update the coop document
    await databases.updateDocument(
      DATABASE_ID,
      COLLECTION_ID_COOPERATIVES,
      coop.$id,
    ...
    try {
      ...
      console.error("Failed to write removal audit log:", logErr);
    }
