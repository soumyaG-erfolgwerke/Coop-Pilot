...
// Import React hooks for state, effect, and ref management
 ...
 * Supports multi-cooperative searching, debounced queries, keyboard navigation (Up/Down/Enter),
 * category filter tabs, and direct routing.
 */
export default function GlobalSearchModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        console.error("Multi-coop search fetch error:", err);
      } finally {
        setLoading(false);
      }
    }, 250);
  ...
  // 6. Direct click-through routing to source record page
  ...
  return (
    ...
    >
      ...
      <div
        className="w-full max-w-2xl bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]"
      ...
      >
        ...
        <div className="flex items-center px-4 py-3.5 border-b border-gray-100 dark:border-neutral-800 bg-white dark:bg-neutral-900">
          ...
          <input
            ...
            placeholder={
              language === "de"
                ? "Genossenschaften durchsuchen (z.B. Weekly, Satzung, Max)..."
                : "Search cooperatives (e.g. Weekly, Satzung, Max)..."
            }
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-4">
          {!query.trim() ? (
            <div className="py-12 text-center text-sm text-gray-400 dark:text-neutral-500">
              {language === "de"
                ? "Geben Sie einen Suchbegriff ein, um alle Ihre Genossenschaften zu durchsuchen."
                : "Type a query to search across all your managed cooperatives."}
            </div>
          ) : !loading && !hasAnyResults ? (
            <div className="py-12 text-center text-sm text-gray-400 dark:text-neutral-500">
              {language === "de"
                ? `Keine Ergebnisse für "${query}" in Ihren Genossenschaften gefunden.`
                : `No records found matching "${query}" in your cooperatives.`}
            </div>
          ) : (
            <>
              {/* 📄 Documents Group */}
