...
export  const RadioGroup = ({ legend, name, options, value, onChange, layout = 'row' }) => (
    <fieldset>
        ...
        <div className={`flex gap-x-6 gap-y-3 ${layout === 'col' ? 'flex-col' : 'flex-col sm:flex-row'}`}>
            {options.map(option => (
                <div key={option.value} className="flex items-center">
                    ...
                    />
                    <label htmlFor={`${name}-${option.value}`} className="ml-3 block text-sm text-gray-800">
                        {option.label}
                    </label>
                </div>
            ))}
                ...
                {isStringURL && (
                    <>
                        <a
                            ...
                            className="px-3 py-2 text-blue-600 border border-primary rounded-md hover:bg-blue-50 flex items-center"
                        >
                        ...
                        <button
                            onClick={onDelete}
                            type="button"
                            className="px-3 py-2 text-red-600 border border-red-500 rounded-md hover:bg-red-50 flex items-center"
                        >
