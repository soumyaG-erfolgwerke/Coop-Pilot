...
import { AlertTriangle, Bell, CheckCheck, Lightbulb, Sparkles, Wrench } from "lucide-react";
...
export default function WhatsNewPanel({ open, onClose, announcements, unreadCount, onMarkAllRead }) {
  ...
  return (
    <>
      <button type="button" aria-label="Close announcements" onClick={onClose} className="fixed inset-0 z-[55] cursor-default bg-transparent" />
      <aside className="absolute right-0 top-full z-[60] mt-3 flex max-h-[min(36rem,calc(100vh-6rem))] w-[min(24rem,calc(100vw-1rem))] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900" aria-label="What's new announcements">
        <header className="flex items-center justify-between border-b border-slate-200 px-5 py-4 dark:border-slate-800">
          ...
          {unreadCount > 0 && <span className="rounded-full bg-rose-100 px-2 py-1 text-xs font-semibold text-rose-700 dark:bg-rose-950 dark:text-rose-300">{unreadCount} new</span>}
        </header>
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
          ...
          {announcements.map((item) => {
            ...
            return <article key={item._id} className="rounded-xl border border-slate-200 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-800/40">
