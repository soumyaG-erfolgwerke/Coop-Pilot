...
// const Page5 = ({ formData, handleChange, errors, selectedCooperative }) => {
const Page5 = ({ formData, handleChange, errors }) => {
  ...
  React.useEffect(() => {
    if (
      ...
      const nameParts = formData.directorName.trim().split(" ");
      if (nameParts.length > 0) {
        // Last part is last name, rest is first and middle name
        const lastName = nameParts[nameParts.length - 1];
        const firstMiddleName = nameParts.slice(0, -1).join(" ");

        // Create synthetic events to update the form
        handleChange({
          target: { name: "fullLegalFirstMiddleName", value: firstMiddleName },
        });
        handleChange({
          target: { name: "fullLegalLastName", value: lastName },
        });
      }
    }
  }, [
          ...
          <div className="flex gap-2">
            ...
            <div className="relative w-32">
              <select
                ...
                onChange={handleChange}
                className={`mt-1 block w-full py-2.5 pl-3 pr-8 border ${
