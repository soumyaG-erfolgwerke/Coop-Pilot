...
export default function WhatsNewManager() {
  ...
  return <section className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)]">
    <div className="rounded-xl border border-slate-800 bg-slate-900 p-6">
      ...
      <label className="mt-5 block text-sm text-slate-300">Title<input maxLength={120} value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 outline-none focus:border-cyan-500" /></label>
      <label className="mt-4 block text-sm text-slate-300">Type<select value={form.type} onChange={(event) => setForm({ ...form, type: event.target.value })} className="mt-2 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 [color-scheme:dark]"><option>New</option><option>Improvement</option><option>Fixed</option><option>Important</option></select></label>
      <label className="mt-4 block text-sm text-slate-300">Message<textarea maxLength={2000} rows={7} value={form.message} onChange={(event) => setForm({ ...form, message: event.target.value })} className="mt-2 w-full resize-y rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 leading-6 outline-none focus:border-cyan-500" /><span className="mt-1 block text-right text-xs text-slate-500">{form.message.length}/2000</span></label>
